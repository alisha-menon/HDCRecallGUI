...
function [prog_HV, result_AM, known_condition_integers] = hdcrorbtrain (features_condition, features_result, chAMcondition, chAMresult, iMchcondition, iMchresult, D, precision_condition, precision_result, channels_condition, channels_result)
%
...
%   result_AM          : AM for result vectors
    ...
    prog_HVlist = zeros(length(features_condition),D);
    
    i = 1;
    y = 1;
    while i <= length(features_condition)
        already_exists = 0;
        ...
        if (already_exists == 0)
            ...
            for x = 1:1:channels_result
                [CiM_result, result_integer] = lookupItemMemeory (chAMresult, features_result(i,x), precision_result);
...
function [actuator_values] = hdcrorbpredict (prog_HV, result_AM, features_condition, chAMcondition, chAMresult, iMchcondition, iMchresult, D, precision_condition, precision_result, channels_condition, channels_result)
%
...
%
    ...
    actuator_values = zeros(length(features_condition),channels_result);
