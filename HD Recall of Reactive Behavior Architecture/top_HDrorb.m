...
%maxL_condition_list = [30]; % # of vectors in CiM
%maxL_result_list = [30]; % # of vectors in CiM
...
%num_result_channels_list = [10];
%num_condition_channels_list = [20];
...
%     non_repeat_errors = [];
%     [row_wrong, col_wrong] = find(expected ~= actuator_values);
%     [~,u,c] = unique(quantized_condition_training);
...
%     for i = 1:1:length(wrong_recall_values_condition)
%         x = find(repeat_values_condition == wrong_recall_values_condition(i));
%         sizex = size(x);
%         if (sizex(1) == 1)
...
%         end
%     end
%     [~,u,c] = unique(quantized_result_training);
%     [n,~,bin] = histcounts(c,numel(u));
%     repeat_bin_address = find(n > 1);
...
%     for i = 1:1:length(wrong_recall_values_condition)
%         x = find(repeat_values_result == wrong_recall_values_result(i));
%         sizex = size(x);
%         if (sizex(1) == 1)
...
%     result_repeat_count = zeros(length(repeat_values_condition),4);
%     for i = 1:1:length(repeat_values_condition)
%         location = find(quantized_condition_training == repeat_values_condition(i));
%         unique_values = unique(quantized_result_training(location));
