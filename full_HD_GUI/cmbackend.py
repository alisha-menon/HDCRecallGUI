        ...
        while self._running:
            ...
            if data == False:
                ...
                if self.updateMdLbl:
                    ...
                    # print('{0:b}'.format((self.mode<<5)+self.label))
...
class streamAdcThread(QThread):
    ...
    def setMdLbl(self,mdlbl):
        mode = mdlbl[0]
        label = mdlbl[1]
        # print('Mode: {}\t Label: {}'.format(mode,label))
    ...
    def run(self):
        ...
        os.makedirs('data/mat', exist_ok=True)
        ...
        while self._running:
            ...
            if not timeout:
                ...
                if data[0]==0x00: # no CRC
                    ...
                    values = [(data[2*(i+1) + 1] << 8 | data[2*(i+1)]) & 0xFFFF for i in range(0,64)]
                    out.append(values)
                    msbs = [(values[i] & 0x8000) >> 15 for i in range(0,64)]
                    # hdout.append(int(''.join(map(str, msbs)), 2) << 1)
                    hdout.append(msbs)
