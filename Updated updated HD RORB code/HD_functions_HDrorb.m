...
function [prog_HV, result_AM] = hdcrorbtrain (length_training, features_condition, features_result, chAMcondition, chAMresult, iMchcondition, iMchresult, D, precision_condition, precision_result, channels_condition, channels_result)
%
...
%   result_AM          : AM for result vectors
    ...
    while i <= length_training
        for x = 1:1:channels_condition
            ...
            %already_exists = find(known_condition == condition_integer);
            %size_exists = size(already_exists);
            %if (size_exists(2) == 0)
                 condition_vectorlist(x,:) = xor(CiM_condition , iMchcondition(x));
            %    known_condition = [known_condition condition_integer];
            %end
        end
        ...
        for m = 1:1:channels_result
            [CiM_result, result_integer] = lookupItemMemeory (chAMresult, features_result(i,m), precision_result);
            result_vectorlist(m,:) = xor(CiM_result , iMchresult(m));
        end
    ...
    prog_HV = mode(prog_HVlist);
end

function [actuator_values] = hdcrorbpredict (length_recall, prog_HV, result_AM, features_condition, chAMcondition, chAMresult, iMchcondition, iMchresult, D, precision_condition, precision_result, channels_condition, channels_result)
%
